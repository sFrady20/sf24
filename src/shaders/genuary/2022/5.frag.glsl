...
void grain(inout vec4 col,inout vec2 uv){
  col=mix(col,grainCol,hash(vec3(uv,1.)).x*.03);
}

void flames(inout vec4 col,inout vec2 uv){
  //float nt = pow( fract(time / 10.), 2. ) * 2. - 1.;
  float nt=sin(time/5.)*2.-1.;
  float amt=pow(pow((fbm(uv*6.,8)*.5+.5),5.)*sdBox(uv,vec2(.5))*100.,(1.-nt)*3.)-nt*2.;
  col=vec4(vec3(1.,.25,.08)/amt,1.);
}
...
void main(){
  ...
  uv-=resolution*.5;// center
  uv/=resolution.x*aspect*.35;//scale
